...
static void mask(const struct piece* const meeple)
{
	...
	al_draw_scaled_bitmap(resource_manager_icon(ICON_ID_MEEPLE),
		0, 0, 512, 512,
		-half_width, -half_height, 2 * half_width, 2 * half_height, 
		0);
}
...
struct piece* meeple_new(lua_State* L)
{
	...
	*meeple = (struct meeple)
	...
	piece->widget_interface->render_interface->half_width = 40;
	piece->widget_interface->render_interface->half_height = 40;
